...
class ThriftParser:
    ...
    RESERVED_PYTHON_KEYWORDS = {
        ...
        # Not hard keywords, but shadowing these stdlib builtins as an
        # attribute name is worth avoiding for readability/tooling.
        "hash", "range", "list", "dict", "set", "str", "int", "float", "bool",
        "bytes", "object", "len", "min", "max", "sum", "all", "any", "map", "filter",
    }
    ...
    def _parse_enum(self):
        ...
        self.expect(self.T_RBRACE)
        if name not in self.enums:  # first definition wins, see _parse_struct
    ...
    def _parse_struct(self, kind="struct"):
        ...
        self.expect(self.T_RBRACE)
        # First definition wins across multiple --input files (and within a
        # single file): earlier files/occurrences take priority, later ones
        # only fill in names not already seen. This matters when the same
        # struct name is defined in more than one input with different field
        # names for the same field id -- e.g. `Message` field 1 is `from` in
        # line.thrift (LINE's own app, matching what the rest of LINEPY's
        # hand-written code expects as `.from_`) but `_from` in chrline.thrift
        # (community-reconstructed, appears twice there too). Passing
        # line.thrift before chrline.thrift on the command line, combined
        # with this first-wins rule, keeps the historically-correct name.
    ...
    def generate_code(self, root_service: Optional[str] = None) -> str:
        ...
        # Generate code
        #
        # Models are plain stdlib `@dataclass`es built on top of
        # `linepy.models.base.ModelBase` -- NOT pydantic. pydantic-core (a
        # Rust extension) has no iOS wheels and no pure-Python fallback
        # (confirmed unplanned upstream), so environments that can't build
        # Rust extensions (e.g. a-Shell on iOS) can never install real
        # pydantic. This project's actual usage of pydantic v2 was always a
        # narrow slice -- aliased fields with defaults, model_validate,
        # model_dump(by_alias=True), TypeAdapter for List/Dict responses, no
        # validators/constraints/computed fields -- which `ModelBase`
        # reproduces exactly on stdlib `dataclasses`. See
        # linepy/models/base.py for the full rationale.
        lines = []
        ...
        lines.append("from .base import ModelBase, model_field")
        lines.append("")
        ...
        for name in sorted(list(all_types)):
            ...
            # kw_only=True: every field always gets a default (see below), so
            # dataclass's "no-default field after a defaulted one" ordering
            # rule is never actually violated here -- but every construction
            # site (ModelBase.from_dict, hand-written `Cls(field=...)` calls)
            # already only ever uses keyword arguments, so kw_only costs
            # nothing and removes the whole class of ordering bugs for free.
            lines.append("@dataclass(kw_only=True)")
            lines.append(f"class {name}(ModelBase):")
            ...
            for field in struct_def["fields"]:
                ...
                lines.append(
                    f"    {fname}: {py_type} = model_field(alias=\"{field['id']}\", {default_val_code})"
                )
    ...
    def generate_service(
        self, service_name: str, endpoint: str = "/S4", models_module: str = "..models.generated"
    ) -> str:
        # Names referenced by the generated signatures, collected as we go so
        # the file can import exactly what it uses instead of star-importing
        # all ~3000 generated models (which made every name in these files
        # unresolvable to linters and IDEs alike).
        used_models = set()
        ...
        def header():
            ...
            known = set(self.structs) | set(self.enums)
            names = sorted(n for n in used_models if n in known)
            if names:
                head.append(f"from {models_module} import (")
                head.extend(f"    {n}," for n in names)
                head.append(")")
            base_module = ".base" if models_module.startswith("..") else ".services.base"
            head.append(f"from {base_module} import ServiceBase")
            head.append("")
            return head
        ...
        for m in methods:
            ...
            used_models.update(self._model_names_in(res_type_py))
    ...
    def _model_names_in(py_type: str):
        ...
        ``Optional[List["SquareEvent"]]`` -> {"Optional", "List", "SquareEvent"}.
        Callers keep only the ones that name a parsed struct/enum."""
        return set(re.findall(r"[A-Za-z_][A-Za-z0-9_]*", py_type))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Import path the generated service uses for models "
             "(default: ..models.generated, for files under linepy/services/)",
    )
    ...
    if args.service and args.service_output:
        code = thrift_parser.generate_service(
            args.service,
            endpoint=args.endpoint or "/S4",
